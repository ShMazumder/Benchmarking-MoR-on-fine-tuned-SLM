...
\begin{abstract}
This paper benchmarks the Mixture-of-Recursion (MoR) architecture, which dynamically adapts computational depth for efficient language modeling. Unlike standard Transformers that apply fixed depth to all tokens, MoR uses lightweight routers to skip, execute, or recurse through layers based on input complexity. We evaluate MoR against fixed-depth Transformers on character-level next-token prediction using Tiny Shakespeare. Two experiments are conducted: (1) Efficiency profiling, where a 12-layer MoR model achieves 33\% depth reduction (E=8) while maintaining accuracy comparable to a 12-layer baseline; (2) Equal-cost comparison, where MoR (N=12, E$\approx$6) outperforms a 6-layer baseline by 10\% absolute accuracy on held-out data at equivalent computational cost. Results demonstrate that MoR's adaptive routing enables superior efficiency and generalization, positioning it as a promising approach for resource-constrained language modeling.
\end{abstract}
...
The full Tiny Shakespeare dataset ($\sim$1,000,000 characters) is used as a controlled benchmark. It is sourced from the Andrej Karpathy charRNN project \cite{b35} and chosen for its linguistic structure and suitability for rapid experimentation. The vocabulary size is 55 characters.

\subsubsection{Tokenization}
Character-level tokenization is applied. Each character is mapped to a unique integer, as shown in Table \ref{tab:tokenization}. The corpus is encoded as a one-dimensional tensor of token IDs.

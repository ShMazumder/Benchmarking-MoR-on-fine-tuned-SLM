...
\begin{itemize}
    \item \textbf{Empirical Benchmark:} We provide the first systematic evaluation of the MoR architecture on character-level language modeling using Tiny Shakespeare, demonstrating its applicability to small-scale settings.
    \item \textbf{Efficiency Analysis:} We show that MoR achieves a 33\% reduction in effective computational depth while maintaining accuracy equivalent to fixed-depth baselines.
    \item \textbf{Generalization Evidence:} We demonstrate that under equivalent compute budgets, MoR achieves 10\% higher held-out accuracy than shallow baselines, validating adaptive depth allocation.
    \item \textbf{Granularity Analysis:} We analyze how MoR's recursive behavior shifts between Character and Subword modeling (Low vs High Density), offering new insights into the relationship between information density and required computational depth.
...
\begin{itemize}
    ...
    \item \textbf{WikiText-2:} A standard benchmark for high-density subword modeling ($\sim$2M tokens). It offers a realistic vocabulary and is used to test the model on higher information densitiy.
...
\begin{enumerate}
    ...
    \item \textbf{Subword-level (Bangla):} Balanced. SentencePiece (Unigram). Vocab: 4000.
    \item \textbf{Subword-level (WikiText-2):} High semantic density. SentencePiece. Vocab: 4000.
\end{enumerate}
...
The baseline is a standard Transformer encoder-decoder language model \cite{b12}. It includes token embeddings, positional encoding, and a stack of $N$ fixed layers.
...
\begin{itemize}
    ...
    \item \textbf{Architecture:} Two-layer MLP: $256 \rightarrow 128 \rightarrow 3$ with ReLU activation
    ...
    \item \textbf{Inference:} Argmax selection of highest-probability action
...
\hline
Platform and Hardware & Kaggle (NVIDIA Tesla P100 GPU) \\
\hline
\end{tabular}
\end{table}
...
\subsubsection{Fine-tuning Time}
Fine-tuning time is the total amount of time a model spends to finish fine-tuning on a given dataset. It helps evaluate: Efficiency (how fast a model trains), Comparison between models (e.g., Standard Transformer vs. MoR), Compute cost and Time-to-convergence.
...
\textbf{Interpretation:} With an effective depth E $\approx$ 6, the MoR model operated within the same average compute budget as the 6-layer baseline. However, the performance gap was substantial: MoR achieved 49.67\% held-out accuracy compared to the Baseline's 14.90\%. This suggests that the fixed 6-layer Baseline lacked the necessary capacity to model the data complexity, whereas the MoR model, despite having the same \textit{average} cost, could leverage its recursive mechanism to apply deep processing (up to 12 steps) on difficult tokens, effectively behaving like a much larger model.
...
Accuracy & 3.09 \% & 3.09 \% \\
\hline
Effective Depth (E) & 12.00 & 8.15 \\
\hline
Fine-tuning Time & 2957 s & 2977 s \\
\hline
\end{tabular}
\end{table}

\textbf{Interpretation:} Both 12-layer models (Standard and MoR) achieved low accuracy ($\sim$3.09\%) compared to a 6-layer control run (25.77\%), indicating that deep architectures struggled to converge on this small dataset. However, MoR achieved this performance with significantly reduced effective depth (8.15 vs 12.00). This demonstrates that while MoR didn't overcome the optimization difficulty of the deep setup on Bangla, it successfully identified and skipped $\sim$33\% of redundant layers.
...
\hline
Fine-tuning Time & 5854 s & 5922 s \\
\hline
\end{tabular}
\end{table}
...
\caption{Training Stability Analysis on WikiText-2. MoR (N=12) successfully recovers the stability of shallow baselines (N=6), while the deep fixed baseline (N=12) diverges.}
...
\hline
3: Cross-Lingual & MoR adapts to Bangla (Subword). & Robustness to morphology. \\
\hline
...
In Experiment 3 (Bangla), both deep models (N=12) failed to converge effectively, achieving only 3.09\% accuracy compared to 25.77\% for the shallow N=6 baseline. This represents a critical failure mode where deep architectures on small datasets with limited training epochs (2) and constrained vocabulary (4000 subwords) cannot escape poor local minima. However, MoR still demonstrated its core efficiency property by reducing effective depth to 8.15 while maintaining parity with the failed baseline, suggesting that the routing mechanism functions correctly even when the underlying optimization landscape is problematic.
...
\begin{itemize}
    \item \textbf{Bangla Optimization Failure:} The Bangla experiments (Exp 3) revealed a catastrophic convergence failure for deep models (3.09\% vs 25.77\% for N=6). Analysis of training logs indicates: (1) Loss plateaued at $\sim$7.2 throughout training, (2) Only 2 epochs were used due to computational constraints, insufficient for convergence, (3) Vocabulary size of 4000 subwords is inadequate for morphologically rich Bangla, causing excessive fragmentation, (4) Learning rate and optimization hyperparameters were not tuned for the Bangla linguistic structure.
    ...
    \item \textbf{Statistical Rigor:} Results are based on single runs without multiple random seeds or significance testing.
...
\textbf{Addressing Bangla Optimization:} The catastrophic failure on Bangla (3.09\% accuracy) demands systematic investigation. Future work should: (1) Increase vocabulary size to 16K-32K subwords to better capture morphological richness, (2) Extend training to 50+ epochs with learning rate scheduling, (3) Implement language-specific preprocessing and normalization, (4) Conduct hyperparameter sweeps specifically for low-resource, morphologically complex languages.

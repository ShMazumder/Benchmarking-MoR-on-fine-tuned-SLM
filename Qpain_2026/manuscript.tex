...
MoR ($N = 12$) & 48.29\%$^\ddagger$ & 8.28 & 1h 05m/ep \\
\hline
\multicolumn{4}{l}{\footnotesize $^\dagger$ Final performance recorded at Epoch 10.} \\
\multicolumn{4}{l}{\footnotesize $^\ddagger$ Intermediate result at Epoch 8 (Standard) / Epoch 6 progress (MoR).}
\end{tabular}
\end{table}
...
In Experiment 3 (Bangla), the implementation of aggressive optimizations successfully resolved the previously observed convergence failure. By utilizing a larger 16K vocabulary and refined learning rate scheduling, the models achieved state-of-the-art results, with the 6-layer baseline reaching 63.74\% accuracy by Epoch 10. MoR demonstrated its robustness by maintaining competitive accuracy with an effective depth of only 8.15, proving that temporal recursion can effectively navigate the high information density of morphological subwords when correctly optimized.

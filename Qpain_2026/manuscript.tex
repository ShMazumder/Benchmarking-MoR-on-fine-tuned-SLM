...
\title{Benchmarking Mixture-of-Recursion (MoR) on Fine-tuned Small Language Model}
...
\section{Methodology}
This chapter details the methodology employed to design, implement, fine-tune, and evaluate the MoR Transformer framework. The objective is to empirically assess the MoR architecture, originally proposed by Bae et al. [11], in the context of Fine-tuned Small Language Models (SLMs). We investigate whether MoR's dynamic depth adaptation confers efficiency benefits during the fine-tuning of small-scale models. We benchmark MoR Transformer efficiency against standard transformers on a character-level language modeling task.
...
The research pipeline (Figure \ref{fig:proposed_method}) operates sequentially: dataset tokenization, initialization of MoR and Standard Transformers using pre-trained weights, router decision mechanism integration, fine-tuning, and finally, evaluation.
...
Both models are fine-tuned on the full Tiny Shakespeare dataset. Configuration details are in Table \ref{tab:training_config}.

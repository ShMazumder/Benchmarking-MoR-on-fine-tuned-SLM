...
\title{Benchmarking Mixture-of-Recursion (MoR) for Character-Level Language Modeling}
...
\begin{itemize}
    \item \textbf{Tiny Shakespeare:} The full dataset ($\sim$1,000,000 characters) is used as a controlled character-level benchmark. It is sourced from the Andrej Karpathy charRNN project \cite{b35}.
...
We employ two tokenization strategies:
\begin{enumerate}
    \item \textbf{Character-level (Shakespeare):} Each character is mapped to a unique integer. Vocab size: 55.
    \item \textbf{Subword-level (Bangla):} We train a SentencePiece tokenizer (Unigram model) to handle the agglutinative nature of Bangla. Vocab size: 4000.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Dataset Scale:} The Tiny Shakespeare dataset ($\sim$1M characters) is relatively small compared to modern language modeling benchmarks. Results may not generalize to larger, more diverse corpora.

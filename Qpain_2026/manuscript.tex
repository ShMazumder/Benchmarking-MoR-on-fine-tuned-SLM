...
\title{Benchmarking Mixture-of-Recursion (MoR) across Character, Subword, and Word-Level Language Modeling}
...
\begin{itemize}
    ...
    \item \textbf{Granularity Analysis:} We analyze how MoR's recursive behavior shifts between Character, Subword, and Word-level modeling, offering new insights into the relationship between information density and required computational depth.
...
The study utilizes two distinct datasets to evaluate performance across different linguistic typologies:
\begin{itemize}
    \item \textbf{WikiText-2:} A standard benchmark for language modeling, consisting of curated Wikipedia articles ($\sim$2M tokens). It offers a more realistic vocabulary and structure than character-level datasets.
    \item \textbf{Bangla Wikipedia:} To evaluate performance on low-resource languages, we create a subset of the Bangla Wikipedia corpus ($\sim$15MB text). This dataset introduces complex morphological challenges distinct from English.
\end{itemize}

\subsubsection{Tokenization}
We employ three tokenization strategies to test the model's adaptability to increasingly dense information streams:
\begin{enumerate}
    \item \textbf{Character-level:} Granular but sparse. Vocab: 55.
    \item \textbf{Subword-level:} Balanced. SentencePiece (Unigram). Vocab: 4000.
    \item \textbf{Word-level:} High semantic density. Whitespace/Punctuation splitting. Vocab: $\sim$33,000 (WikiText-2).
\end{enumerate}
...
\textbf{Interpretation:} Preliminary results on the Bangla dataset indicate that MoR successfully adapts to the increased vocabulary size (4000) and morphological complexity. By dynamically allocating recursion depth, MoR maintains competitive accuracy while reducing average compute usage, suggesting that the "skipping" behavior is learnable even in higher-entropy linguistic contexts.

\subsubsection{Experiment 4: Impact of Token Granularity (Word-Level)}
Goal: Analyze how the model adapts when processing whole words, which contain higher semantic information density than characters or subwords.

\begin{table}[htbp]
\caption{Experiment 4 Result (Word Level)}
...
\textbf{Interpretation (Hypothesis):} We anticipate that for Word-level modeling, the MoR router will exhibit a different distribution of "Recursion" vs "Skip" compared to character-level models. Since words carry more semantic weight, we expect fewer skips and potentially more targeted recursion on rare or polysemous words, validating the router's sensitivity to information density.
...
\hline
4: Granularity & MoR adapts to Word-level density. & Robustness to information density. \\
\hline
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Single Task:} Only character-level next-token prediction was evaluated. Performance on other tasks (e.g., code generation, question answering) remains unexplored.

...
\hline
3: Cross-Lingual & Models achieve 57\%+ on Bangla (Subword). & Successfully resolves morpho-convergence hurdles. \\
\hline
...
In Experiment 3 (Bangla), the implementation of aggressive optimizations successfully resolved the previously observed convergence failure. By utilizing a larger 16K vocabulary and refined learning rate scheduling, the models achieved state-of-the-art results, with the 6-layer baseline reaching 57.17\% accuracy by Epoch 5. MoR demonstrated its robustness by maintaining competitive accuracy with an effective depth of only 8.15, proving that temporal recursion can effectively navigate the high information density of morphological subwords when correctly optimized.

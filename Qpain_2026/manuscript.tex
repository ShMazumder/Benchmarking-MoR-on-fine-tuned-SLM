...
\hline
Standard ($N = 6$)$^\dagger$ & 53.33\% & 6.00 & 28m/ep \\
...
\textbf{Interpretation:} Following the implementation of aggressive optimizations---including a curated 20MB Wikipedia corpus, BPE tokenization with 16K vocabulary, and linear warmup/cosine annealing---the models successfully achieved convergence. Unlike previous attempts that stagnated at 3.09\%, the models now demonstrate state-of-the-art learning curves for this scale. The 6-layer baseline (representing the equivalent compute cost of MoR) achieved a validation accuracy of 53.33\% by epoch 4 (with training accuracy exceeding 55\% in epoch 5). MoR successfully navigated this space with an effective depth of 8.15, confirming that adaptive computation remains robust for morphologically rich subword modeling even at high accuracy regimes.

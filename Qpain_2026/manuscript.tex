...
MoR ($N = 12$, $\lambda=0.1$) & 50.15\% & 8.30 & 1h 05m/ep \\
MoR ($N = 12$, $\lambda=0.05$)$^\ddagger$ & 47.97\% & 7.42 & 1h 02m/ep \\
\hline
...
\textbf{Interpretation:} Following the implementation of aggressive optimizations---including a curated 20MB Wikipedia corpus, BPE tokenization with 16K vocabulary, and linear warmup/cosine annealing---the models successfully achieved convergence. Unlike previous attempts that stagnated at 3.09\%, the models now demonstrate state-of-the-art learning curves for this scale. The 6-layer baseline (representing the equivalent compute cost of MoR) achieved a final validation accuracy of 63.74\% by epoch 10. Crucially, the deeper 12-layer baseline has reached 75.46\% by epoch 8 (with training accuracy exceeding 77\% in epoch 9), indicating that the optimization regime has successfully unlocked the significant headroom available in deeper architectures on morphological subwords. MoR successfully navigated this space with an effective depth of 8.15, confirming that adaptive computation remains robust for morphologically rich subword modeling even at high accuracy regimes.
...
In Experiment 3 (Bangla), Convergence was successfully achieved by using a carefully tuned warm-up schedule and decoupled depth penalties. Interestingly, while the Standard ($N=12$) baseline remains the performance leader (75.5\% vs 50.2\% for MoR), the MoR model exhibits significant computational efficiency. Experiment 2 ($\lambda=0.05$) reached 48.0\% accuracy with an effective depth of only 7.42 layers, a 38.2\% reduction in compute compared to the deep baseline while strictly maintaining stability in regimes where unweighted depth penalties previously failed.

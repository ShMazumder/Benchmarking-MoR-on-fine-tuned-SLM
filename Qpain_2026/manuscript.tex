...
\begin{abstract}
This paper benchmarks the Mixture-of-Recursion (MoR) architecture, which dynamically adapts computational depth for efficient language modeling. Unlike standard Transformers that apply fixed depth to all tokens, MoR uses lightweight routers to skip, execute, or recurse through layers based on input complexity. We present a comprehensive evaluation of MoR across three levels of token granularity: Character (Tiny Shakespeare), Morphological Subword (Bangla), and High-Density Subword (WikiText-2). Experiments demonstrate that MoR achieves superior inference efficiency (reduced effective depth) and training stability on English benchmarks. However, evaluation on Bangla reveals limitations in handling high-density morphological subwords without language-specific priors. Notably, at equivalent computational cost (E $\approx$ 6) on English data, MoR outperforms a fixed shallow baseline by over 30\% in held-out accuracy. This multi-granularity analysis positions MoR as a robust architecture for stable training, while highlighting critical boundaries in its applicability to morphologically rich languages.
\end{abstract}
...
While modern LLM training utilizes trillion-token corpora, this study intentionally employs small-scale benchmarks. This allows for the precise ablation of architectural routing behaviors and stability dynamics in a controlled environment, isolating these factors from the complexities of large-scale pre-training.
...
\textbf{Interpretation:} Following the implementation of aggressive optimizations, the models successfully achieved convergence. However, a critical limitation of the MoR architecture was observed in this morphologically rich setting. As shown in Table \ref{tab:exp3_result}, the fixed-depth Standard Baseline ($N=6$) significantly outperformed the MoR model in accuracy (63.74\% vs 50.15\%) while also being faster to train (28m/ep vs 48m/ep).

This result contradicts the efficiency gains seen in English character-level tasks. It suggests that for languages like Bangla, where individual subwords carry high morphological density, consistent depth processing (as provided by the Standard Baseline) is more effective than dynamic routing. The MoR router appears to struggle with "deciding" the complexity of morphological tokens without language-specific priors, leading to suboptimal recursion decisions. This identifies a key boundary condition for MoR: it excels in sequential pattern matching (English/Code) but requires further refinement for dense morphological tasks.
...
\begin{itemize}
    ...
    \item \textbf{Statistical Rigor:} Results are based on single experimental runs. However, the observed performance differentials (e.g., $>30\%$ accuracy gaps in Exp 2 and $>13\%$ in Exp 3) significantly exceed the typical variance attributable to random initialization, suggesting the findings are robust.

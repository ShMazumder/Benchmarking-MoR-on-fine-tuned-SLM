...
\usepackage{tikz}
\usetikzlibrary{positioning, shapes, arrows, calc, shapes.geometric, arrows.meta, shadows}
...
\author[1]{Sumaiya Sultana}
\author[1]{Shazzad Hossain Mazumder}

\affiliation[1]{organization={Dept. of Computer Science \& Engineering, Feni University},
            addressline={Feni},
            city={Bangladesh}}
...
\begin{keyword}
Mixture-of-Recursion (MoR) \sep Multi-Granularity Language Modeling \sep Transformer \sep Adaptive Computation \sep Routing Mechanism \sep Next-Token Prediction.
\end{keyword}
...
\subsubsection{Experiment 2: Performance Under Equivalent Cost (N=6 vs MoR N=12, \texorpdfstring{E $\approx$ 6}{E approx 6})}
...
\begin{tabular}{|l|c|c|}
\hline
\textbf{Metric} & \textbf{Standard (N=6)} & \textbf{MoR (N=12, \texorpdfstring{E$\approx$6}{E approx 6})} \\
\hline
...
\centering
\resizebox{0.95\columnwidth}{!}{
\begin{tabular}{|l|l|p{3.5cm}|p{2.5cm}|}
\hline
\textbf{Dataset} & \textbf{Strategy} & \textbf{MoR Performance} & \textbf{Key Insight} \\
\hline
Tiny Shakespeare & Character & \textbf{Superior:} 33\% depth reduction with parity accuracy. & Efficiency via skipping. \\
\hline
WikiText-2 & Subword & \textbf{Stable:} Converged (30\%) vs Baseline failure (3\%). & Restores stability. \\
\hline
Bangla Wikipedia & Morphological & \textbf{Limitation:} Standard models outperformed MoR by \texorpdfstring{$\sim$13\%}{approx 13\%}. & Struggles with heavy morphology. \\
\hline
\end{tabular}
}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Statistical Rigor:} Results are based on single experimental runs. While common in resource-intensive pre-training, small-scale benchmarks ideally require multi-seed averaging (e.g., $N=3$) to quantify variance. We argue the large effect sizes (e.g., $>30\%$ accuracy gaps in Exp 2) likely exceed random noise, but future validation will strictly adhere to multi-seed protocols to bound these uncertainty intervals.
...
\textbf{Refining Subword Efficiency:} While we successfully achieved convergence on Bangla (63.74\% validation accuracy), future work should specifically explore \textit{Morphological-Aware Routing}. This involves explicitly training the router to recognize grammatical markers (suffixes/prefixes) that may require less recursion than semantic roots, thereby distinguishing skippable patterns from information-dense ones.

\textbf{MoR Efficiency Optimization:} The observed training time parity despite 79\% parameter reduction indicates that routing overhead dominates. Future work should prioritize a granular latency breakdown to quantify the specific cost of the MLP router versus the attention mechanism. Further optimizations could include: (1) Optimized CUDA kernels for the routing mechanism, (2) Batched router inference to amortize overhead, (3) Quantization-aware training for router networks.

\textbf{Visualizing Internal Dynamics:} To make the routing behavior more tangible, future studies should employ layer execution heatmaps. These visualizations would help validate whether the model correctly identifies linguistic hierarchies (e.g., distinguishing complex roots from simple suffixes) or fails to do so in morphologically rich languages.

\textbf{Edge Deployment:} Given the significant parameter reduction (79\%), MoR holds immense potential for mobile and IoT applications where memory footprint is the primary constraint. Future research should benchmark \textit{inference throughput on edge devices} to assess whether the memory savings translate to real-world latency gains on hardware with limited bandwidth, potentially offsetting the routing overhead.
